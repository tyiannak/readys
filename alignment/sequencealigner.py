...
class Scoring(object):
    ...
    @abstractmethod
    def __call__(self, firstElement, secondElement,a,b):
        return 0
...
class SimpleScoring(Scoring):
    ...
    def __call__(self, firstElement, secondElement,a,b):
        if firstElement == secondElement:
            #print(self.matchScore)
            return self.matchScore
        else:
         
            self.mismatchScore = Levenshtein.ratio(a,b)
            #print(self.mismatchScore)
...
class SequenceAligner(object):
    ...
    def align(self, first, second,a,b, backtrace=False):
        f = self.computeAlignmentMatrix(first, second,a,b)
        score = self.bestScore(f)
        if backtrace:
            alignments = self.backtrace(first, second, f,a,b)
            return f,score, alignments
        else:
            return score
    ...
    @abstractmethod
    def computeAlignmentMatrix(self, first, second,a,b):
        return numpy.zeros(0, float)
    ...
    @abstractmethod
    def backtrace(self, first, second, f,a,b):
        return list()
...
class GlobalSequenceAligner(SequenceAligner):
    ...
    def computeAlignmentMatrix(self, first, second,a,b):
        m = len(first) + 1
        n = len(second) + 1
        f = numpy.zeros((m, n), float)
        for i in range(1, m):
            for j in range(1, n):
                # Match elements.
                ab = f[i - 1, j - 1] \
                    + self.scoring(first[i - 1], second[j - 1],a[i-1],b[j-1])
    ...
    def backtrace(self, first, second, f,a,b):
        ...
        self.backtraceFrom(first, second, f, m - 1, n - 1,
                           alignments, alignment,a,b)
        return alignments

    def backtraceFrom(self, first, second, f, i, j, alignments, alignment,d,e):
        if i == 0 or j == 0:
